...
import { Layout, Menu, Image, Card, Button } from "antd";
...
class LoginHome extends React.Component {
  ...
  //   };
  redirectToProductDisplay = (item) => () => {
    // e.preventDefault();
    // console.log(item);
    const { name } = item;
    localStorage.setItem("product_obj", JSON.stringify(item));

    const { history } = this.props;
    if (history) history.push(`/loginhome/${name}`);
  };
  render() {
    return (
      <div>
              ...
              return (
                <ul key={item.id}>
                  ...
                  <Button
                    ...
                    value={item}
                    type="primary"
                    onClick={this.redirectToProductDisplay(item)}
                  >
